...
definitief as (
    ...
    select
        ...
        -- Datums
        koppelingen.startdatum_locatie  as startdatum,
        koppelingen.einddatum_locatie   as einddatum,

        -- Actueel vandaag
        case
            when (koppelingen.startdatum_locatie is null or koppelingen.startdatum_locatie <= cast(getdate() as date))
             and (koppelingen.einddatum_locatie  is null or koppelingen.einddatum_locatie  >= cast(getdate() as date))
            then 1 else 0
        end as is_actief
