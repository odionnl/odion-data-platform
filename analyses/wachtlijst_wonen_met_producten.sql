...
-- TODO: onderscheid maken tussen wachtlijst wonen + wachtlijst logeren

-- Materialiseer actieve_koppelingen als temp table (wordt 3x gebruikt)
drop table if exists #actieve_koppelingen;
...
into #actieve_koppelingen

from [OdionDataPlatform].[dbo].[stg_onsdb__location_assignments] lk
    inner join [OdionDataPlatform].[dbo].[int_locatie_hierarchie] l
    on l.locatie_id = lk.locatie_id;

create index ix_ak_client on #actieve_koppelingen (client_id);
...
with
    ...
    (
        ...
        where cluster = N'Wachtlijsten'
            and niveau3 in (N'LG (Wachtlijst)', N'VG (Wachtlijst)')
...
select
    ...
    max(case when ak.cluster = N'Kind en gezin'           then 1 else 0 end) as [Kind en gezin],
    max(case when ak.cluster = N'Multidisciplinair Team'  then 1 else 0 end) as [Multidisciplinair Team]

from wachtlijst_clienten wl
    inner join [OdionDataPlatform].[dbo].[mart_clienten] c
